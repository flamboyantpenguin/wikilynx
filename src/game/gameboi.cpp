...
GameBoi::GameBoi(ScoreSheet *gameData, QString levelName, QString playerName,
                 QString league)
    : QObject() {
  ...
  this->levelInfo = this->gameData->getLevel(levelName);
  this->levels = levelInfo["levels"].toString().split(" ");
  qDebug() << levels;

  // Intialize Sound Subsystem
  soundSystem = new Oscillator();

  // Set Player Name. Watch The Good, The Bad and The Ugly (1966) to understand
  // what Blondie means
...
void GameBoi::emitClicks() {
  int totalClicks = levelInfo["clicks"].toInt();

  QString clicksText = "$c $t";
  clicksText.replace("$c", QString::number(clicks));
  if (totalClicks)
    clicksText.replace("$t", "/ " + QString::number(totalClicks));
  else
    clicksText.replace("$t", "");
...
void GameBoi::click(QString url) {
  ...
  if (levelInfo["wiki?"].toBool())
    url = url.split("wikipedia.org/wiki/")[1];
...
void GameBoi::updateCountdown() {
  ...
  double time = levelInfo["time"].toDouble();

  // Send counter text to the GUI. This is can perhaps be shifted to the GUI
  // itself. Opinions are welcome
  QString counterText = "$c $t";
  counterText.replace("$c", QString::number(countup, 'f', 2));
  if ((time != 0.00))
    counterText.replace("$t", "/ " + QString::number(time, 'f', 2));
  else
    counterText.replace("$t", "");
  emit counter(counterText);

  // Timeout!
  if (time > 0 && countup >= time)
    endGame(1);

  // Max Clicks Reached!
  int allowedClicks = this->gameData->getLevel(levelName, "clicks").toInt();
  if (allowedClicks > 0 && clicks > allowedClicks)
    endGame(2);
}

int GameBoi::getChk() { return chk; }

QStringList GameBoi::getLevels() { return this->levels; }
...
void GameBoi::endGame(int code) {
  ...
  gameStat["clicks"] = QString::number(last_clicks);
  gameStat["chk"] = QString::number(chk);
  gameStat["timeTaken"] = QString::number(last_countup, 'f', 4);
  gameStat["league"] = this->league;
  gameStat["status"] = code;

  if (code == 1)
    soundSystem->playSound("yay");
  else
    soundSystem->playSound("error");
  ...
  if (code == 0)
    gameData->appendPlayerStats(levelName, gamer,
                                QString::number(countup, 'f', 4));
}
